...
def corr(x, y):
    ...
    cov = tf.tensordot(tf.constant(x), tf.constant(y.T), 1) - n * tf.tensordot(tf.constant(mu_x[:, np.newaxis]), tf.constant(mu_y[np.newaxis, :]), 1)
	#return cov / np.dot(s_x[:, np.newaxis], s_y[np.newaxis, :])
    return tf.divide(cov, tf.tensordot(tf.constant(s_x[:, np.newaxis]), tf.constant(s_y[np.newaxis, :]), 1)).eval(session=sess)
