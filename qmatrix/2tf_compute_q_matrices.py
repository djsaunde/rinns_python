...
def corr(x, y):
	...
	return np.dot(x_mx, y_my.T) / np.sqrt(np.dot(ssx[:,None], ssy[None]))
...
print('We are started with rank', rank) # TODO

# the first rank does all the "heavy lifting"
if rank == 0:
	...
	for layer_idx in range(num_layers, 1, -1):
		print('Computing Q-matrix for layer ' + str(layer_idx - 1))
		...
		qfname_temp = 'q' + str(layer_idx - 1) + '_temp.npy'
		qfname = 'q' + str(layer_idx - 1) + '.npy'
		...
		if 'data_a_map' in locals():
			...
			data_b = np.load(os.path.join(activation_path, 'layer' + str(layer_idx - 1) + '.npy')).T
			data_b = data_b.reshape((np.prod(data_b.shape[:-1]), data_b.shape[-1]))
	
		data_a = np.load(os.path.join(activation_path, 'layer' + str(layer_idx - 2) + '.npy')).T
		...
		print('Building Q' + str(layer_idx - 1) + ' (' + str(q_size[0]) + 'x' + str(q_size[1]) + ')')
		...
		if has_weights and use_weights:
			...
			np.save(os.path.join('q_out', 'w' + str(layer_idx - 1) + '.npy'), to_write)
			del to_write
			...
			weights = np.load(os.path.join('q_out', 'w' + str(layer_idx - 1) + '.npy'))
			...
			os.remove(os.path.join('q_out', qfname_temp))
